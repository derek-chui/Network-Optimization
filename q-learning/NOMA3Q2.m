        ...
        for i = 1:length(indices)
            for j = i+1:length(indices)
                for k = j+1:length(indices)
                    ...
                    sc = tripletUtility(idx, points, weights);
                    if sc > bestScore
                        bestScore = sc;
                        bestGroup = idx;
                    end
...
function score = calcScore(groupingSet, points, weights)
    ...
    for i = 1:size(groupingSet, 1)
        score = score + tripletUtility(groupingSet(i, :), points, weights);
    end
end
...
function rew = tripletUtility(idx, points, weights)
    ...
    hj = h_sorted(1); hm = h_sorted(2); hk = h_sorted(3);
    ...
    %SINR & SIC
    gamma_j = (alpha1*P*hj^2) / ((alpha2+alpha3)*P*hj^2 + N0);
    gamma_m = (alpha2*P*hm^2) / (alpha3*P*hm^2 + N0);
    gamma_k = (alpha3*P*hk^2) / N0;

    %SEMANTIC FIDELITY PARAMS
    a1 = 0.30; a2 = 0.98; c1 = 0.25; c2 = -0.8;

    %SEMANTIC FIDELITY
    xi_j = a1 + (a2-a1) ./ (1 + exp(-(c1*gamma_j + c2)));
    xi_m = a1 + (a2-a1) ./ (1 + exp(-(c1*gamma_m + c2)));
    xi_k = a1 + (a2-a1) ./ (1 + exp(-(c1*gamma_k + c2)));

    %SEMANTIC IMPORTANCE PARAMS
    kk = weights(idx);
    kk = kk(ord);
    kj = kk(1); km = kk(2); kk_ = kk(3);

    %SEMANTIC IMPORTANCE
    Gj = kj * xi_j * log2(1 + gamma_j);
    Gm = km * xi_m * log2(1 + gamma_m);
    Gk = kk_ * xi_k * log2(1 + gamma_k);
