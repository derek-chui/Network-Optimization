%Multiple, Joint Power, Pinch Antenna, Semantic, Q-Learning, NOMA
...
USE_Q_ASSIGNMENT = true; %rl chooses antenna->pair labels if true
...
if M < 4
    error('M must be >= 4 for 4 arms (N/S/E/W).');
end
...
%distribute remainder across arms
nE = base_n + (rem_n >= 1);
nW = base_n + (rem_n >= 2);
nN = base_n + (rem_n >= 3);
nS = base_n;
...
fprintf('Arm counts: E=%d, W=%d, N=%d, S=%d\n', nE, nW, nN, nS);
...
title(sprintf('Pinch Antennas (Multiple Waveguides)'));
