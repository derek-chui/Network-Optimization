%Circular, Joint Power, Pinch Antenna, Semantic, Q-Learning, NOMA
...
% Scan grid of alpha values and pick alpha* that maximizes semantic utility for that grouping
% join power so alpha grid for weak user's share
USE_JOINT_ALPHA = true;

%alpha_w in [0.5, 0.95] means weak gets alpha_w and strong gets 1-alpha_w
%include 0.5/0.5 and options like 0.7/0.3 etc.
ALPHA_GRID = 0.50:0.05:0.95;

%if u ever want to force a fixed alpha and disable joint then set USE_JOINT_ALPHA=false and pick:
alpha_fixed_weak = 0.60;
...
for i=1:K
    text(U(i,1)+2, U(i,2), sprintf('U%d (w=%.2f)', i, w_sem(i)));
end
...
title(sprintf('Pinch Antennas SQ-NOMA (Joint Alpha = %d): Total = %.4f', USE_JOINT_ALPHA, sum(pair_util)));
